import { BunnyRoot } from "./BunnyRoot";
 ...
 */
export default class BunnyElement<T extends HTMLElement> extends BunnyRoot<T> {
    /**
     * Crea un nuevo elemento del tipo especificado
     * @param tagName Nombre de la etiqueta HTML a crear
     */
    constructor(tagName: string) {
        super(document.createElement(tagName) as T);
    }
     ...
     * @returns La instancia actual para encadenamiento
     */
    attr(attrs: Record<string, string> | string): string | null {
        if (typeof attrs === "string") return this.element.getAttribute(attrs);
